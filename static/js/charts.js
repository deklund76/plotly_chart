...
function buildCharts(sample) {
  ...
  d3.json("static/data/samples.json").then((data) => {
    ...
    var selectedSample = samples.filter(selected => selected.id == sample)[0];
    ...
    var bubbleData = [{
      ...
      mode: "markers",
      marker: {
        color: otu_ids.map(val => 
          `rgb(${val % 255}, ${val*2 % 255}, ${val*7 % 255})`),
        size: sample_values.map(val => 125*val/Math.max(...sample_values))
      }
    }];
